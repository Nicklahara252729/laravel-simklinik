...
class EloquentKartuPasienRepositories implements KartuPasienRepositories
{
    ...
    public function get($noRm, $uuidFaskes)
    {
        try {
            ...
            /**
             * convert tanggal daftar
             */
            $tglDaftar = Carbon::parse($getPasien->created_at)->locale('id');
            $tglDaftar->settings(['formatFunction' => 'translatedFormat']);

            /**
             * count ages
             */
            $bornDate = Carbon::parse($getPasien->tgl_lahir)->locale('id');
            $nowDate = Carbon::now();
            $age = $bornDate->diffInYears($nowDate);
            ...
            $data = [
                ...
                "email" => $getPasien->email,
                "gender" => $getPasien->gender,
                "golongan_darah" => $getPasien->golongan_darah,
                "no_hp" => $getPasien->no_hp_1,
                "no_rm" => $getPasien->no_rm,
                "tgl_daftar" => $tglDaftar->format('j F Y'),
                "umur" => $age,
                "jenis_layanan" => null,
                "foto" => $getPasien->foto
            ];
