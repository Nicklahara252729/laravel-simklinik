...
const getAllData = async () => {
    try {
        ...
        $("#table-poliklinik").on("click", ".call-button", async function() {
            const queue = this.dataset.queue;
            console.log(queue);
            await callQueue(queue);
        })
        ...
        if(user.level == 'perawat') {
            ...
            $("date-filter").find('input').prop('disabled', true);
        }
...
const renderDataPoliklinik = async (UUIDPoliklinikLinkKlinik, dateValue = null) => {
    try {
        ...
        await renderDataTable(dataPoliklinik);
    } catch (error) {
        console.error(error);
    }
};
...
const getDataById = async (uuidPoliklinik) => {
    try {
        const response = await sendApiRequest(
            {
                url: `api/web/poliklinik/get/pemeriksaan/${uuidPoliklinik}`,
                type: "GET",
            },
            true
        );
