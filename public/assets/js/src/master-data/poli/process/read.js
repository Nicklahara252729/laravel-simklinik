    ...
    const table = $("#table-poliklinik").DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                render: (data) => {
                    const { uuid_poliklinik: uuidPoliklinik, poliklinik } = data;
                    return `
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                            <div class="menu-item px-3">
                                <a href="javascript:void(0)" data-uuid=${uuidPoliklinik} class="menu-link px-3 edit-data-button">Edit</a>
                            </div>
                            <div class="menu-item px-3">
                                <a href="javascript:void(0)" data-uuid=${uuidPoliklinik} data-nama="${poliklinik}" class="menu-link px-3 delete-data-button" data-kt-users-table-filter="delete_row">Delete</a>
                            </div>
                        </div>
                    `;
                },
...
const renderDataPoliklinik = async () => {
    const dataPoliklinik = await getDataPoliklinik();
    renderDataTable(dataPoliklinik);
}

const getDataById = async (uuid_poliklinik) => {
    try {
        const response = await sendApiRequest({
            url: `/api/web/master-data/poli/get/${uuid_poliklinik}`,
            type: "GET",
        }, true);
        return response.data;
    } catch (error) {
...
export { renderDataPoliklinik, getAllData, getDataById, getDataPoliklinik };
